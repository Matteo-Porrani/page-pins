...
<template>
	<ul class="w-[60%] flex flex-col gap-2 overflow-y-auto py-2 mx-auto">
		...
		>
			...
			<p
				...
				:class="{
					'bg-white' : 0 !== $s.colorizeData.currColorId,
					'bg-sky-100' : 0 === $s.colorizeData.currColorId,
				}"
			>
				none
			</p>
		...
		>
			...
			<p
				class="w-[160px] rounded-lg py-1 px-3"
				:class="{
					'bg-white' : c.id !== $s.colorizeData.currColorId,
					'bg-sky-100' : c.id === $s.colorizeData.currColorId,
				}"
			>
				{{ c.name }}
			</p>
